...
def test_fading_cf1():
    ...
    assert(np.linalg.norm(estimated_fading - expected_fading) < TOL)
    ...
    assert(np.linalg.norm(estimated_cf1 - expected_cf1) < TOL)
...
def test_radius():
    ...
    assert(np.abs(estimated_radius - expected_radius) < TOL)
...
def test_adding_updating():
    ...
    assert(np.linalg.norm(mc.time_array - expected_time_array) < TOL)
    ...
    assert(np.abs(mc.weight - expected_weight) < TOL)
    assert(np.linalg.norm(mc.center - expected_center) < TOL)


def test_update_parameters_same():
    """
    ...
    assert(np.abs(c1.weight - c2.weight) < TOL)
    assert(np.linalg.norm(c1.center - c2.center) < TOL)
