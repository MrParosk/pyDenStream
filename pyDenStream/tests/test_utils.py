...
def test_fading_function():
    ...
    lambd = 1.0
    time = 2
    assert(np.abs(utils.fading_function(lambd, time) - 0.25) < TOL)
    ...
    assert(np.linalg.norm(actual_array - expected_array) < TOL)


def test_cf1_calculations():
    """
    ...
    assert(np.linalg.norm(np_cf1 - numba_cf1) < TOL)


def test_cf2_calculations():
    """
    ...
    assert(np.linalg.norm(np_cf2 - numba_cf2) < TOL)
